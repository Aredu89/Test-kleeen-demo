...
function Workflow({ translate, ...props }) {
  const taskName = `publisherDetails`;
  ...
  const workflowName = `PublisherDetails`;
  const objectFocus = `publisher`;
  function handleOnTabIndexChanged(newTabIndex, option) {
    setSelectedViewOption(option);
  }
  const publisherActions = useKleeenActions(taskName);
        ...
        >
          ...
          >
            <DataViewControlSection
              ...
              entity={entity}
              entityActions={publisherActions}
              hideRefreshControl
              isEntityDetails
              objectValue={objectFocus}
