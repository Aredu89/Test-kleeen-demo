...
function Workflow({ translate, ...props }) {
  const taskName = `departmentDetails`;
  const entity = `BookStoreDepartment`;
  ...
  const workflowName = `Department Details`;
  const objectFocus = `bookStoreDepartment`;
  function handleOnTabIndexChanged(newTabIndex, option) {
    setSelectedViewOption(option);
  }
  const bookStoreDepartmentActions = useKleeenActions(taskName);
        ...
        >
          ...
          >
            <DataViewControlSection
              ...
              entity={entity}
              entityActions={bookStoreDepartmentActions}
              hideRefreshControl
              isEntityDetails
              objectValue={objectFocus}
